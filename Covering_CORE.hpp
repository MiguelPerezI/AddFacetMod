...
class Covering {
	...
	void runRCC(SubComplexJ L, Complex K, SimplicialMap a, SimplicialMap b, int M, double r, int argc, char **argv) {
		...
		//	cout << "\n->AddFACET Finding Simplex to join\n";	
			simplexScrutiny(count, L, K, a, b, M, r, argc, argv);
	...
	void simplexScrutiny(int stb, SubComplexJ L, Complex K, SimplicialMap a, SimplicialMap b, int M, double r, int argc, char **argv) {
							//////////////////
							//////////////////
							//////RUN RCC/////
							//////////////////
		...
		while (  cycleRCC < 4) {
			...
			if(core == 0) {
				a.escListInt(O);
				//cout << "\ncpu(" << core << ") eats simplex(" << RR0 << ") in O\n";
				cout << "\n\n";
			}
